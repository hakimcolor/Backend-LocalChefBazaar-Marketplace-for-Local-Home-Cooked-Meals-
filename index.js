  ...
  try {
    ...
    app.get('/orders/delivered/count', async (req, res) => {
      try {
        ...
        res.json({ success: true, deliveredOrders: deliveredCount });
      } catch (error) {
        console.error(error);
        res
          .status(500)
    ...
    app.get('/orders/pending-payment/count', async (req, res) => {
      try {
        ...
        res.json({ success: true, pendingPayments: pendingCount });
      } catch (error) {
        console.error(error);
        res
          .status(500)
    ...
    app.get('/users/count', async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, message: 'Failed to get total users' });
      }
    });
    ...
    app.get('/orders/paid/total', async (req, res) => {
      try {
        ...
        res.json(result[0] || { totalPaidAmount: 0, totalOrders: 0 });
      } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'Server Error' });
      }
    ...
    app.post('/create-checkout-session', async (req, res) => {
      ...
      try {
          ...
          line_items: [
            {
              price_data: {
                currency: 'usd',
                product_data: { name: name || 'Food Order' },
                unit_amount: amount * 100,
              },
              quantity: 1,
            },
        ...
        res
          .status(500)
          .json({ success: false, message: 'Stripe session error' });
      }
    });
    ...
    app.put('/meals/:id', async (req, res) => {
      ...
      try {
        ...
        res.json({
          success: true,
          updatedMeal: normalizeDoc(updatedMeal.value),
        });
      } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, error: error.message });
    ...
    app.get('/reviews/:mealId', async (req, res) => {
      ...
      try {
        const reviews = await reviewsCollection
          ...
          .sort({ date: -1 })
          .toArray();
        res.json({ success: true, data: reviews.map(normalizeDoc) });
      } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, error: error.message });
      }
    ...
    app.patch('/reviewsup/:id', async (req, res) => {
      ...
      try {
        ...
        res.json({
          success: true,
          updatedReview: normalizeDoc(updatedReview.value),
        });
      } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, error: error.message });
    ...
    app.get('/favorites/:email', async (req, res) => {
      ...
      try {
        const favorites = await favoritesCollection
          .find({ userEmail: email })
          .toArray();
        res.json({ success: true, data: favorites.map(normalizeDoc) });
      } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, error: error.message });
      }
    ...
    // ----------------- Test Route -----------------
    app.get('/', (req, res) => res.send('Hello World from Express + MongoDB!'));
  } finally {
    // No need to close client; server keeps running
  }
}
